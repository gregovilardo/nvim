...
vim.api.nvim_create_autocmd("BufWritePost", {
  ...
  callback = function()
    ...
    if not path:find(NOTES_DIR, 1, true) then return end
...
function M.start()
  ...
  local mappings = {
    ["1"] = function() close() M.new_note(nil) end,
    ["2"] = function() close() require("oil").open(NOTES_DIR, { preview = {} }) end,
    ["3"] = function() close() require("telescope.builtin").find_files({ cwd = NOTES_DIR }) end,
    ["4"] = function() close() M.open_last() end,
    ["q"] = close,
  }
...
vim.keymap.set("n", "<leader>nn", function() M.new_note(nil) end, { desc = "[N]otes: [N]ew note" })
vim.keymap.set("n", "<leader>nl", function() M.open_last() end, { desc = "[N]otes: [L]ast note" })
vim.keymap.set("n", "<leader>nd", function() M.start() end, { desc = "[N]otes: [D]ashboard" })
